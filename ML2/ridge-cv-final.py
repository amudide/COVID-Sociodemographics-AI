...
y1 = (df[['Log-Cases-Adj']]).values.tolist()
y2 = (df[['Log-Deaths-Adj']]).values.tolist()

print(len(X))
print(len(y1))
print(len(y2))

reg1 = linear_model.RidgeCV(alphas=[1e-3, 1e-2, 1e-1, 1, 1e1, 1e2, 1e3], cv=5)
reg1.fit(X, y1)

print("\n\n LOG CASES \n\n")
print("R^2 Score:", reg1.score(X, y1))
...
reg2 = linear_model.RidgeCV(alphas=[1e-3, 1e-2, 1e-1, 1, 1e1, 1e2, 1e3], cv=5)
reg2.fit(X, y2)

print("\n\n LOG DEATHS \n\n")
print("R^2 Score:", reg2.score(X, y2))
