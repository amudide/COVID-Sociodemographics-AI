...
reg1 = linear_model.RidgeCV(alphas=[1e-3, 1e-2, 1e-1, 1])

from sklearn.model_selection import cross_val_score
tmp1 = linear_model.Ridge(alpha=0.01)
scores = cross_val_score(tmp1, X, y1, cv=10)
...
print("CV Values:", reg1.best_score_)

reg2 = linear_model.RidgeCV(alphas=[1e-2], store_cv_values=True)
# print(reg2.best_score_)
scores = cross_val_score(reg2, X, y2, cv=10)
